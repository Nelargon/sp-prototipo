...
export function buscar(datos, idx, consulta, limite = 40) {
  ...
  // Solo cuentan los huecos del master (Silver y Gold). La columna de
  // Essential sale de su cuadernillo, y su "-1" es "preguntale al asesor",
  // no un hueco de la grilla: no debe mover el orden.
  const incompleta = (it) => (it.s[0] === -1 || it.o[0] === -1 ? 0.25 : 0);
