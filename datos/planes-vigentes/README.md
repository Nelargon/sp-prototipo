...
| `essential.json` | **Essential (desde el 24/09/2026 reemplaza a Bronze).** Precio por zona (Asunción y Central · Interior · Nacional), reglas de grupo, cobertura clave y esperas, transcriptos de `PRECIOS ESSENTIAL.pdf` (21/08/2026) y `CUADERNILLO PLAN ESSENTIAL.pdf` (25/03/2026), con la contradicción de la tabla editable anotada. Otra estructura que Privilege: no entra en la grilla; su columna de `lib/prestaciones.json` sale de `essential-cobertura.json`. |
| `essential-cobertura.json` | **Essential estudio por estudio (25/09/2026).** Reglas transcriptas del `CUADERNILLO PLAN ESSENTIAL.pdf` (25/03/2026), cada una con su cláusula: patrones sobre los nombres de la grilla (gana la primera que calza; las dudosas «?» van primero), mapeo de las 43 especialidades, exclusiones y números finos. Lo que el cuadernillo no nombra → «No entra en este plan»; lo dudoso → «Confirmalo con tu asesor» (26 ítems, pendientes de SP). `scripts/build-prestaciones.mjs` la lee y **corta el build si un patrón no calza con ninguna fila**. |
