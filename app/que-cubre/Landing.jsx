...
/* /que-cubre — el espacio propio de los planes Essential, Silver y Gold.
...
const seccionTitulo = (kicker, titulo, resalte, bajada) => (
  <div data-rv style={css('text-align:center;max-width:700px;margin:0 auto 26px')}>
    <div className="disp" style={css('font-size:12px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:var(--sp-teal-deep);margin-bottom:13px')}>{kicker}</div>
    <h2 className="disp" style={css('font-size:clamp(27px,3.6vw,38px);font-weight:800;color:var(--sp-navy);line-height:1.15;letter-spacing:-0.02em;margin:0 0 13px')}>
      {titulo} <span style={css('color:var(--sp-teal-deep)')}>{resalte}</span>
    </h2>
    <p style={css('font-family:var(--font-inter),sans-serif;font-size:16.5px;line-height:1.6;color:var(--sp-text);margin:0')}>{bajada}</p>
  </div>
);
...
export default function Landing() {
  ...
  const badge = (c) =>
    'display:inline-flex;align-items:center;font-size:12px;font-weight:700;padding:3px 10px;border-radius:var(--r-pill);white-space:nowrap;' +
    (c.ok ? 'background:var(--sp-mint-bg);color:var(--sp-teal-deep)' : 'background:var(--sp-gold-bg);color:var(--sp-gold-ink)');
  ...
  return (
    <div ref={raiz} className="body tactil" style={css('min-height:100vh;background:#fff;color:var(--sp-ink)')}>
      ...
      <section style={css('padding:100px 24px 12px')}>
        <div style={css('max-width:1080px;margin:0 auto')}>
          <div style={css('text-align:center;max-width:720px;margin:0 auto 26px')}>
            <div className="disp" style={css('font-size:12px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:var(--sp-teal-deep);margin-bottom:14px')}>Essential · Silver · Gold</div>
            <h1 className="disp" style={css('font-size:clamp(32px,5vw,50px);font-weight:800;color:var(--sp-navy);line-height:1.1;letter-spacing:-0.025em;margin:0 0 16px')}>
              Tres planes que <span style={css('color:var(--sp-teal-deep)')}>podés revisar antes de firmar</span>.
            </h1>
            <p style={css('font-family:var(--font-inter),sans-serif;font-size:17.5px;line-height:1.6;color:var(--sp-text);margin:0')}>
              Preguntá por el estudio que te pidió el doctor, por la cirugía que te preocupa o por el especialista que buscás. Te decimos qué hace cada plan con eso — <b style={css('color:var(--sp-navy)')}>incluso cuando la respuesta no nos conviene</b>.
            </p>
          </div>
          <Buscador />
        </div>
      ...
      <section style={css('padding:64px 24px 0')}>
        <div style={css('max-width:1080px;margin:0 auto')}>
          {seccionTitulo('Los tres planes', 'Qué trae cada uno', 'y cuánto sale.', 'Silver y Gold cuestan lo mismo en todo el país; Essential, según tu zona. Todos con IVA incluido y, con débito automático o tarjeta de crédito, 10% menos.')}
          ...
          <div style={css('font-family:var(--font-inter),sans-serif;font-size:12.5px;color:var(--sp-muted);text-align:center;margin-top:13px')}>
            ...
            {[
              ...
              <div key={s.a} data-rv className="rv sq" style={css('background:var(--sp-mint-tint);border:1px solid var(--sp-mint-line);--sq:var(--r-lg);padding:24px 26px')}>
                ...
                <div style={css('display:flex;align-items:baseline;gap:10px;flex-wrap:wrap')}>
                  <span className="disp num-tnum" style={css('font-size:44px;font-weight:800;color:var(--sp-teal-deep);line-height:1;letter-spacing:-0.03em')}>{s.d.total}</span>
                  <span style={css('font-family:var(--font-inter),sans-serif;font-size:15px;color:var(--sp-text);line-height:1.45')}>cosas mejoran</span>
                </div>
                <div style={css('font-family:var(--font-inter),sans-serif;font-size:14px;color:var(--sp-text);line-height:1.6;margin-top:12px')}>
                  El salto está sobre todo en <b style={css('color:var(--sp-navy)')}>{nombreCuadro(s.d.donde[0].c)}</b> ({s.d.donde[0].n} de los {s.d.total}).
                  {s.d.desdeConvenio > 0 && (
                    <> Y <b style={css('color:var(--sp-navy)')}>{s.d.desdeConvenio}</b> pasan de pagarlos vos enteros a tener cobertura.</>
                  )}
                </div>
                <div style={css('display:flex;flex-wrap:wrap;gap:6px;margin-top:14px')}>
                  {s.d.donde.map((d) => (
                    <span key={d.c} className="disp" style={css('font-size:11.5px;font-weight:700;padding:4px 10px;border-radius:var(--r-pill);background:#fff;border:1px solid var(--sp-mint-line-strong);color:var(--sp-teal-900)')}>
                  ...
                  {PLAN_KEYS.map((k) => {
                    ...
                    return (
                      <div key={k} style={css('padding:12px;text-align:center;border-left:1px solid var(--sp-line-2);font-family:var(--font-inter),sans-serif;font-size:13px;color:var(--sp-text);line-height:1.4')}>
                        {texto}
                        {copago && <div className="disp" style={css('font-size:11px;font-weight:700;color:var(--sp-estado-ink-2);margin-top:3px')}>Pagás la mitad</div>}
                      </div>
                    );
                  })}
                </div>
      ...
      <section style={css('padding:70px 24px 0')}>
        <div style={css('max-width:1080px;margin:0 auto')}>
          ...
          {seccionTitulo('Antes de firmar', 'Lo que nuestros planes', 'no cubren.', 'Preferimos que lo sepas ahora y no en la sala de espera. Valen para los tres planes, con una diferencia: Essential cubre la odontología básica en Lister.')}
      ...
      <section style={css('padding:44px 24px 80px')}>
        <div style={css('max-width:1080px;margin:0 auto')}>
          ...
          <div style={css('display:flex;flex-wrap:wrap;justify-content:center;gap:10px 26px;margin-top:28px')}>
            <a href={`${BP}/`} className="link-teal disp" style={css('display:inline-flex;align-items:center;gap:7px;color:var(--sp-teal-deep);font-size:14px;font-weight:700')}>
            ...
            <a href={`${BP}/planes/`} className="link-teal disp" style={css('display:inline-flex;align-items:center;gap:7px;color:var(--sp-teal-deep);font-size:14px;font-weight:700')}>
