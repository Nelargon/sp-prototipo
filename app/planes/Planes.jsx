...
/* /planes — la página de los tres planes (la familia que internamente se llama
   "Privilege"; de cara al usuario es Bronze, Silver y Gold — HANDOFF dec. 11o).
   ----------------------------------------------------------------------------
   ...
   con 935 estudios, análisis y cirugías; acá se puede consultar. La página
   completa el argumento en este orden:
     ...
     4. Servicio por servicio→ la comparación de siempre (la que ya existía)
              ...
              <div style={css('display:grid;grid-template-columns:1.5fr 1fr 1fr 1fr;background:#003B71;color:#fff')}>
                ...
                {plansArr.map((pl, i) => (
                  <div key={i} style={css('padding:14px 12px;text-align:center;border-left:1px solid rgba(255,255,255,0.12)')}>
                    ...
                    <a href={`${BP}/simulador/?plan=${pl.short.toLowerCase()}`} onClick={() => track('cta_simulador', { origen: 'planes_tabla', plan: pl.name })} className="disp" style={css('margin-top:9px;height:34px;padding:0 14px;border-radius:9px;background:#007d77;color:#fff;font-size:12.5px;font-weight:700;display:inline-flex;align-items:center;justify-content:center;gap:5px;transition:background .2s')}>Ver mi precio</a>
                  </div>
                ))}
              </div>
              {cov.map((item, r) => (
        ...
        <div style={css('max-width:1080px;margin:0 auto')}>
          ...
          {!verTodas && (
            <div style={css('text-align:center;margin-top:14px')}>
              <button
                type="button"
                onClick={() => { setVerTodas(true); track('planes_ver_especialidades'); }}
                className="disp"
      ...
      <section style={css('padding:44px 24px 80px')}>
        <div style={css('max-width:1080px;margin:0 auto')}>
          <div data-rv style={css('background:#003B71;border-radius:20px;padding:44px 32px;text-align:center')}>
            ...
            <a href={`${BP}/simulador/`} onClick={() => track('cta_simulador', { origen: 'planes_cierre' })} className="btn-teal disp" style={css('height:52px;padding:0 30px;border-radius:13px;background:#007d77;color:#fff;font-size:16px;font-weight:700;display:inline-flex;align-items:center;justify-content:center;gap:8px')}>
              Simulá tu plan
