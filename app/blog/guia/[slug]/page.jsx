    ...
    <div className="body" style={css('min-height:100vh;background:#002A52;color:#fff;display:flex;flex-direction:column')}>
      ...
      <div style={css('flex:1;padding:118px 24px 80px')}>
        <div style={css('max-width:820px;margin:0 auto')}>
          <a href={`${BP}/blog/`} style={css('display:flex;width:fit-content;align-items:center;gap:7px;color:#80DDD8;font-size:14px;font-weight:700;margin-bottom:26px')}>
          ...
          <div style={css('display:inline-flex;align-items:center;gap:8px;font-size:12px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:#80DDD8;border:1px solid rgba(128,221,216,.4);padding:7px 14px;border-radius:999px;margin-bottom:18px')}>Guía</div>
          ...
          <p style={css('font-family:var(--font-inter),-apple-system,BlinkMacSystemFont,Segoe UI,Roboto,Arial,sans-serif;font-size:17px;line-height:1.65;color:#B3C7DB;max-width:60ch;margin:0 0 10px')}>{guia.promesa}</p>

          <div style={css('font-family:var(--font-inter),-apple-system,BlinkMacSystemFont,Segoe UI,Roboto,Arial,sans-serif;font-size:13.5px;color:#7FA3C4;margin:0 0 40px')}>
            ...
            {guia.notas.map((n, i) => (
              <li key={n.slug}>
                <a href={`${BP}/blog/${n.slug}/`} className="guia-item" style={css('display:grid;grid-template-columns:44px 132px 1fr;gap:18px;align-items:center;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.07);border-radius:14px;padding:14px 18px 14px 14px;color:inherit')}>
                  <div className="disp" style={css('font-size:22px;color:#80DDD8;text-align:center')}>{i + 1}</div>
                  <Cover categoria={n.categoria} slug={n.slug} cover={n.cover} alt="" aspect="16 / 10" radius={9} />
                  <div>
                    ...
                    <div style={css('font-family:var(--font-inter),-apple-system,BlinkMacSystemFont,Segoe UI,Roboto,Arial,sans-serif;font-size:12.5px;color:#7FA3C4')}>{n.categoria} · {formatFechaCorta(n.date)} · {n.minutes} min</div>
                  </div>
                </a>
              </li>
            ))}
