...
import { getPublishedPosts, getPost, formatMonthYear } from '../../../lib/blog';
...
export default async function BlogPost({ params }) {
    ...
    return (
      <div className="body" style={css('min-height:100vh;display:flex;align-items:center;justify-content:center;background:#002A52;padding:24px')}>
        <div style={css('background:#fff;border-radius:18px;padding:44px 32px;text-align:center;max-width:460px')}>
          ...
          <p style={css('font-size:15px;color:#6B6B6B;line-height:1.6;margin:0 0 22px')}>Todavía no hay notas publicadas en el blog.</p>
  ...
  return (
    <A kicker={post.kicker} title={post.title} intro={post.intro} minutes={post.minutes} date={formatMonthYear(post.date)}>
