    ...
    <div>
      ...
      {destacada && (
        <div className="blog-top" style={css('display:grid;grid-template-columns:1.62fr 1fr;gap:34px;align-items:start;margin-bottom:44px')}>
          <a href={`${basePath}/blog/${destacada.slug}/`} className="blog-hero" style={css('display:block;color:#fff')}>
            ...
            <div style={css('font-family:var(--font-inter),-apple-system,BlinkMacSystemFont,Segoe UI,Roboto,Arial,sans-serif;font-size:13px;color:var(--sp-blue-meta)')}>{meta(destacada)}</div>
          </a>
      ...
      {cats.length > 1 && (
        <div role="group" aria-label="Filtrar notas por sección" style={css('display:flex;flex-wrap:wrap;gap:9px;align-items:center;padding-top:6px;border-top:1px solid rgba(255,255,255,0.14);margin-bottom:26px')}>
          <span className="disp" style={css('font-size:12px;font-weight:800;letter-spacing:.12em;text-transform:uppercase;color:var(--sp-mint);margin:20px 10px 0 0')}>Secciones</span>
          {chips.map((t) => {
            const on = t === active;
            const c = cuenta(t);
            return (
              <button
                ...
                onClick={() => setActive(t)}
                aria-pressed={on}
                style={css('margin-top:20px;cursor:pointer;font-family:var(--font-display),sans-serif;font-weight:700;font-size:13.5px;padding:9px 16px;border-radius:var(--r-pill);transition:background .2s,border-color .2s,color .2s;border:1px solid ' + (on ? 'var(--sp-teal)' : 'rgba(255,255,255,0.28)') + ';background:' + (on ? 'var(--sp-teal)' : 'transparent') + ';color:' + (on ? '#012a2a' : 'rgba(255,255,255,0.88)'))}
              >
      ...
      <h2 className="disp" style={css('font-size:15px;font-weight:800;letter-spacing:.02em;color:#fff;margin:0 0 16px')}>
        {esPortada ? 'Más notas' : active} <span style={css('color:var(--sp-blue-meta);font-weight:600')}>· {grilla.length}</span>
      </h2>

      <div className="blog-list" style={css('display:grid;grid-template-columns:repeat(3,1fr);gap:16px')}>
        {grilla.map((n) => (
          <a key={n.slug} href={`${basePath}/blog/${n.slug}/`} className="blog-card" style={css('display:flex;flex-direction:column;background:#fff;border-radius:18px;overflow:hidden;color:var(--sp-ink);min-height:300px')}>
            ...
            <div style={css('display:flex;flex-direction:column;flex:1;padding:22px 22px')}>
              ...
              <div className="disp" style={css('font-size:19px;line-height:1.25;letter-spacing:-0.01em;color:var(--sp-navy);margin-bottom:9px')}>{n.title}</div>
              <div style={css('font-family:var(--font-inter),-apple-system,BlinkMacSystemFont,Segoe UI,Roboto,Arial,sans-serif;font-size:13.5px;color:var(--sp-text);line-height:1.55;flex:1')}>{n.description}</div>
              <div style={css('display:flex;align-items:center;justify-content:space-between;gap:10px;margin-top:16px')}>
                <span style={css('font-family:var(--font-inter),-apple-system,BlinkMacSystemFont,Segoe UI,Roboto,Arial,sans-serif;font-size:12px;color:var(--sp-muted)')}>{meta(n)}</span>
                <span style={css('display:inline-flex;align-items:center;gap:6px;font-size:13.5px;font-weight:800;color:var(--sp-teal-deep);white-space:nowrap')}>Leer <svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12h14M13 6l6 6-6 6" /></svg></span>
              </div>
            </div>
          </a>
        ))}
