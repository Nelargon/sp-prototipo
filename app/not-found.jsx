...
const boton = (fondo, color, borde) => css('height:50px;padding:0 22px;border-radius:var(--r-sm);display:inline-flex;align-items:center;justify-content:center;font-size:15px;font-weight:700;background:' + fondo + ';color:' + color + ';border:1.5px solid ' + borde);
    ...
    <div className="body" style={css('min-height:100vh;background:var(--sp-mint-tint);color:var(--sp-ink)')}>
      ...
      <div style={css('max-width:620px;margin:0 auto;padding:130px 20px 80px;text-align:center')}>
        ...
        <div style={css('display:flex;flex-wrap:wrap;gap:10px;justify-content:center')}>
          <a href={`${BP}/simulador/`} className="disp" style={boton('var(--sp-teal-deep)', '#fff', 'var(--sp-teal-deep)')}>Simulá tu plan</a>
          <a href={`${BP}/guia-medica/`} className="disp" style={boton('#fff', 'var(--sp-navy)', 'var(--sp-mint-line-strong)')}>Buscá tu médico</a>
          <a href={`${BP}/`} className="disp" style={boton('#fff', 'var(--sp-navy)', 'var(--sp-mint-line-strong)')}>Ir al inicio</a>
        </div>
      </div>
    </div>
  );
