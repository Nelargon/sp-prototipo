...
export default function Header({ variant = 'dark' }) {
  ...
  return (
    <>
      <nav data-nav className={['tactil', variant === 'solid' ? 'solid' : '', open ? 'menu-open' : ''].filter(Boolean).join(' ')} style={css('position:fixed;top:0;left:0;right:0;z-index:100;display:flex;align-items:center;justify-content:space-between;padding:16px 40px')}>
        ...
        <div style={css('display:flex;align-items:center;gap:16px')}>
          ...
          <div className="nav-links-desktop" style={css('display:flex;align-items:center;gap:26px')}>
            ...
            {CON_GUIA
              ? <a href={GUIA} onClick={() => tGuia('nav')} className="nav-guia-cta sq" style={css('height:40px;padding:0 18px;--sq:var(--r-sm);font-size:14px;font-weight:700;display:inline-flex;align-items:center;gap:7px;white-space:nowrap')}>Guía Médica</a>
          ...
          <button className="nav-burger sq" onClick={() => setOpen((o) => !o)} aria-expanded={open} aria-controls="mobile-menu" aria-label={open ? 'Cerrar menú' : 'Abrir menú'} style={css('display:none;width:40px;height:40px;--sq:var(--r-xs);border:none;background:rgba(255,255,255,0.16);color:#fff;align-items:center;justify-content:center;cursor:pointer;flex:none')}>
            {open
      ...
      {open && (
        <div id="mobile-menu" className="menu-overlay" role="dialog" aria-modal="true" aria-label="Menú">
          <nav style={css('display:flex;flex-direction:column')}>
            ...
            <BotonRevision donde="menu" onClick={close} />
          </nav>
        </div>
      )}
    </>
