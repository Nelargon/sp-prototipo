...
| `sim_zona_sin_lista`  | texto (lo tipeado, normalizado — sin datos personales: viaja solo si parece un lugar, sin números, «@» ni más de 4 palabras; si no, vacío con `descartado: true`) | ciudades que la gente pide y el índice no tiene: candidatas a sumarse a `app/geo.js` y señal temprana de demanda en zonas nuevas |
